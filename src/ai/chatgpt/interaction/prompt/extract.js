...
export async function extractChatGptResponse(page) {
  ...
  const rawText = await extractText(page, lastTurn.locator(".markdown"), null, []);
  ...
  for (const sel of canvasSelectors) {
    const canvasText = await page.locator(sel).last().innerText().catch(() => "");
    if (canvasText && canvasText.trim().length > 10) return cleanAiResponse(canvasText);
  }
  ...
  const assistantTurn = page.locator('[data-message-author-role="assistant"]').last();
  ...
  if (assistantText) {
    ...
    const stripped = assistantText.replace(/<tool-plan>[\s\S]*?<\/tool-plan>/gi, "").trim();
