...
export class SessionManager {
 ...
 logger.info(`[SessionManager] GC sweeping idle session: ${session.id} (idle ${Math.round((now - lastUsed) / 60000)}m)`);
 ...
 .setMode(mode)
 .catch((err) =>
 logger.warn(`[SessionManager] setMode(${mode}) failed: ${err.message}`),
 );
 }
 ...
 await session.engine.setMode(mode).catch((err) =>
 logger.warn(`[SessionManager] setMode(${mode}) failed: ${err.message}`)
 );
 }
 }

 logger.info(`[SessionManager] Session ${session.id} created for provider ${providerId} with mode ${mode || "none"}`);
 ...
 logger.info(`[SessionManager] Session ${sessionId.slice(0, 8)} expired after ${Math.round((now - lastUsed) / 60000)}m idle.`);
 this.closeSession(sessionId);
 return null;
 }
 ...
 logger.info(`[SessionManager] ♻️ Recycled ${session.providerId} tab back to pool.`);
 } else {
 if (session.engine && typeof session.engine.close === "function") {
 await session.engine.close().catch((e) =>
