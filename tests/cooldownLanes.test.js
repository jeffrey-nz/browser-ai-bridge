...
const read = (...p) =>
  fs.readFileSync(path.join(HERE, "..", "src", ...p), "utf8");
...
test("a lane is writable exactly when its SITE is — the tri-state still holds", () => {
  ...
  assert.equal(
    cooldownManager.check("gemini:pro").active,
    false,
    "gemini is writable, so its lanes are",
  );
...
test("the cooldown is recorded where the MODE is known — the executor, not promptWorkflow", () => {
  ...
  assert.match(
    read("routes", "ask", "executor", "index.js"),
    /cooldownKey\(session\.providerId, session\.mode\)/,
  );
  assert.doesNotMatch(
    read("ai", "shared", "promptWorkflow.js"),
